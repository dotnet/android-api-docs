...
static class ImporterProgram
{
    ...
    {
        var withoutType = Regex.Replace(
            CleanSourceText(value),
            @"^(?:[\w.$]+(?:<[^>]+>)?(?:\[\])?)\s*:\s*(?=\S)",
            "",
            RegexOptions.CultureInvariant);
        return CleanSourceText(withoutType).Trim();
    }
    ...
    sealed class SourcePage
    {
        ...
        static string HtmlText(string html) =>
            HtmlListText(html) ?? HtmlTextCore(html);

        static string HtmlTableCellText(string html)
        {
            return HtmlListText(html) ?? HtmlTextCore(html);
        }

        static string? HtmlListText(string html)
        {
            ...
            return string.IsNullOrWhiteSpace(listIntroduction)
                ? null
